...
export default ({ vehicle, city, mapBearing, onClick }: { vehicle: Vehicle, city: City, mapBearing: number, onClick?: () => void }) => {
    return <Marker
        ...
        clickTolerance={10}
        style={{ cursor: "pointer", display: "block", transition: "transform 0.1s linear", zIndex: 2 }}
        onClick={onClick}
    >
        <VehicleMarker color={Color(vehicle.type, city)} opacity={vehicle.isPredicted ? 0.8 : 1} backgroundColor={vehicle.isSpecial ? "#F5CF4B" : (vehicle.isEco ? "#83F493" : "#FFF")}>
