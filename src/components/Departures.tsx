    ...
    return departures ? departures.departures.length ? <List>
        {departures.departures.map<React.ReactNode>((departure, i) => <ListItemButton key={i} onClick={() => onClick?.(departure)}>
            <ListItemText
                ...
                secondary={<>{Math.round(departure.delay / 60000) ? <b style={{ color: darkMode ? "#F26663" : "red" }}>{Math.abs(Math.round(departure.delay / 60000))} min {departure.delay > 0 ? "opóźnienia" : "przed czasem"}</b> : <b style={{ color: departure.status === "REALTIME" ? darkMode ? "#90EE90" : "green" : "" }}>{departure.status === "REALTIME" ? "Planowo" : "Według rozkładu"}</b>} · <span style={{ textDecoration: Math.round(departure.delay / 60000) ? "line-through" : "" }}>{new Date(departure.scheduledTime).toLocaleTimeString("pl", { hour12: false, hour: "2-digit", minute: "2-digit" })}</span>{departure.platform && <> · Peron <b>{departure.platform}</b></>}</>}
            />
            <ListItemText
                sx={{ textAlign: "right" }}
